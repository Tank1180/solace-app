...
function businessRequired(req, res, next) {
...
router.get('/quarterly-payments', (req, res) => {
...
router.post('/quarterly-payments', (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM quarterly_tax_payments WHERE id = ?').get(info.lastInsertRowid);
  res.status(201).json({ payment: row });
});

router.delete('/quarterly-payments/:id', (req, res) => {
  db.prepare('DELETE FROM quarterly_tax_payments WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
