...
router.get('/plans', async (req, res) => {
  const plans = await db.prepare('SELECT * FROM subscription_plans ORDER BY customer_type, monthly_price').all();
  res.json({ plans });
});
...
router.post('/plans', async (req, res) => {
  ...
  const plan = await db.prepare('SELECT * FROM subscription_plans WHERE id = ?').get(info.lastInsertRowid);
  res.status(201).json({ plan });
});

router.put('/plans/:id', async (req, res) => {
  const existing = await db.prepare('SELECT * FROM subscription_plans WHERE id = ?').get(req.params.id);
...
router.get('/overview', async (req, res) => {
  ...
  `).all();
  const unassigned = await db.prepare("SELECT customer_type, COUNT(*) as c FROM users WHERE role = 'user' AND plan_id IS NULL GROUP BY customer_type").all();
